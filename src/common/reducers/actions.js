...
export const getAppLatestVersion = async () => {
  const { data: latestReleases } = await axios.get(
    'https://api.github.com/repos/AQRRastiq/arkemysclient/releases?per_page=10'
  );
  ...
  try {
    const rChannel = await fs.readFile(
      path.join(appData, 'arkemysclient_next', 'rChannel')
    );
...
export const checkForPortableUpdates = () => {
  return async (dispatch, getState) => {
    ...
    if (latestVersion) {
      const baseAssetUrl = `https://github.com/AQRRastiq/arkemysclient/releases/download/${latestVersion?.tag_name}`;
